...
async def get_articles(request):
    ...
    # Add debug logging to see the exact response format
    logger.debug(f"Article response format: {json.dumps(response, indent=2)}")
...
async def start_background_tasks(app):
    ...
    logger.info("Clearing Redis cache during initialization...")
